...
RGB_FOLDER = ['frontcamera', 'downcamera']
...
class AirVLNENV:
    ...
    def load_my_datasets(self):
        ...
        if args.obj_desc_json_path is not None:
            with open(args.obj_desc_json_path, 'r', encoding='utf-8') as f: 
                traj_des = json.load(f)
        # list_data_dict = json.load(open(self.eval_json_path, "r"))
        traj_name_path_dict ={item:os.path.join(self.dataset_path, scene_name, item) for scene_name in os.listdir(os.path.join(self.dataset_path)) for item in os.listdir(os.path.join(self.dataset_path, scene_name))}
        traj_names_set = set(traj_name_path_dict.keys())
        ...
        need_traj_names = traj_names_set.difference(skipped_trajectory_set)
        # print("skipped_trajectory_set",skipped_trajectory_set)
        print('Loading dataset metainfo...')
        need_traj_names = sorted(need_traj_names)
        for traj_name in tqdm.tqdm(need_traj_names):
            traj_path = traj_name_path_dict[traj_name]
            path_parts = traj_path.split('/')
            map_name = path_parts[-2]
            seq_name = path_parts[-1]
            mark_json = os.path.join(traj_path, 'mark.json')
            sorted_log_files = sorted(os.listdir(os.path.join(traj_path, 'log')))
            
            # 读取第0个log文件
            log_file = sorted_log_files[0]
            start_log_path = os.path.join(traj_path, 'log', log_file)
            with open(start_log_path, 'r') as f:
                start_log_data = json.load(f)
                uav_position = start_log_data["sensors"]["state"]["position"]
                uav_orientation = start_log_data["sensors"]["state"]["orientation"]
            ...
            frames = [{'position': uav_position, 'orientation': uav_orientation}]
            traj_info['trajectory'] = frames
            traj_info['trajectory_dir'] = traj_path
            traj_info['instruction'] = traj_des[seq_name]
    ...
    def _changeEnv(self, need_change: bool = True):
        ...
        print('using_map_list:', using_map_list)
        ...
        while True:
            try:
                ...
                if self.simulator_tool is not None:
                    print('运行了closeScenes')
                    # self.simulator_tool.closeScenes()
                    self.simulator_tool._destroy_airsim_clients([self.port+i for i in range(1,self.batch_size+1)])  
                    self.simulator_tool = None  # 释放对象
                time.sleep(10)
    ...
    def _getStates(self):
        if args.name == 'qwen2_5vl':
            import time 
            start_time = time.time()
            responses_for_record = self.simulator_tool.getImageResponsesForRecord()
            responses = responses_for_record
            print("getImageResponsesForRecord time:", time.time() - start_time)
        elif args.name == 'qwen2_5vl_256':
            import time 
            start_time = time.time()
            responses = self.simulator_tool.getImageResponses()
            print("getImageResponses time:", time.time() - start_time)
            start_time = time.time()
            responses_for_record = self.simulator_tool.getImageResponsesForRecord()
    ...
    def makeActions_qwenvl(self, waypoints_list):
        waypoints_args = []
        cnt = 0
        for index_1, item in enumerate(self.machines_info):
        ...
        results = self.simulator_tool.move_path_by_waypoints_qwenvl(waypoints_list=waypoints_args)
        ...
        for index_1, item in enumerate(self.machines_info):
            for index_2, _ in enumerate(item['open_scenes']):
                ...
                new_waypoints_list.append(results[index_1][index_2]['states'][0]['sensors']['state']['position'])
                batch_iscollision.append(results[index_1][index_2]['collision'])

        
        for index, waypoints in enumerate(new_waypoints_list):
            for waypoint in waypoints: # check stop
                if np.linalg.norm(np.array(waypoint) - np.array(self.batch[index]['object_position'])) < args.SUCCESSDISTANCE:
            ...
            if batch_iscollision[index]:
                print("{}index collision{}".format(index,batch_iscollision[index]))
                self.sim_states[index].is_end = True
