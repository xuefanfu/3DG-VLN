...
def initialize_env_eval(dataset_path, save_path, eval_json_path, port):
    train_env = AirVLNENV(batch_size=args.batchSize, dataset_path=dataset_path, save_path=save_path, eval_json_path=eval_json_path, port=port)
    return train_env
...
class EvalBatchState:
    def __init__(self, batch_size, env_batchs, env):
        ...
        self.envs_to_pause = []
        self.oracle_success_done = [False] * batch_size
        self.updata_direction = [None] * batch_size
        self.need_rotate = [False] * batch_size
        self.traj_objdes = {}
        # # 读取json文件
        with open(args.target_des_path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                line = line.strip()
                path, scene, traj, obj_des = line.split("\t")
                self.traj_objdes[traj.strip()] = obj_des.strip()
    ...
    def update_metric(self):
        for i in range(self.batch_size):
            ...
            if self.predict_dones[i] and not self.skips[i]:
                self.dones[i] = True
                if self.distance_to_ends[i][-1] <= args.SUCCESSDISTANCE and not self.collisions[i]:
                    self.success[i] = True
                if self.oracle_success[i] and not self.collisions[i]:
                    self.oracle_success_done[i] = True
        ...
        for i in range(self.batch_size):
            ...
            if self.dones[i] and not self.skips[i]:
                ...
                if self.success[i]:
                    prex = 'success_'
                    print(i, " has succeed!")
                elif self.oracle_success_done[i]:
                    prex = "oracle_"
